...
//        int x[] = new int[3];
...

        {
            ...
            case 11:
                x[2] += ((int) k.charAt(p + 10) << 24);

            case 10:
                x[2] += ((int) k.charAt(p + 9) << 16);

            case 9:
                x[2] += ((int) k.charAt(p + 8) << 8);

              /* the first byte of c is reserved for the length */

            case 8:
                x[1] += ((int) k.charAt(p + 7) << 24);

            case 7:
                x[1] += ((int) k.charAt(p + 6) << 16);

            case 6:
                x[1] += ((int) k.charAt(p + 5) << 8);

            case 5:
                x[1] += k.charAt(p + 4);

            case 4:
                x[0] += ((int) k.charAt(p + 3) << 24);

            case 3:
                x[0] += ((int) k.charAt(p + 2) << 16);

            case 2:
                x[0] += ((int) k.charAt(p + 1) << 8);
